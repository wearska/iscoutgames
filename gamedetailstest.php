...
$row = $result->fetch_assoc();
    $gamename = $conn->real_escape_string($row["Name"]);
...
$conn->close();
?>  
        <form id="gameform" action="/backend/addtolibrary.php?ID=<?php echo "$game_id"; ?>" method="post">
            <?php
if(!empty($_SESSION['LoggedIn']) && !empty($_SESSION['Username'])) {
                ...
                $sql = "SELECT games FROM users WHERE Username = '$username'";
                    ...
                    $usergamesstring = implode (" ", $usergames);
                       // echo $usergamesstring;
                        if (strpos($usergamesstring, $gamename) !== false) {
                            echo '<input type="submit" class="disabledsubmit" value="Add to Library" disabled>';
                        } else {
                            echo '<input type="submit" value="Add to Library">'; }
} else {
    echo '<input type="submit" class="remove" value="Remove from Library">'; 
};
            ?>
        </form>
